...
export function renderMap(el, config = {}) {
  ...
  const token = el.dataset.token || window.MAPBOX_TOKEN || null;
...
export function updateMap(el, data, instance) {
  ...
  if (layers && typeof layers === 'object') {
    Object.entries(layers).forEach(([id, value]) => {
      const visibility = value === true ? 'visible' : value === false ? 'none' : value;
      const targets = [id, `${id}-fill`, `${id}-outline`];
