...
**Symptom:** Post has `scatter3D`, `bar3D`, `line3D`, `surface`, or `grid3D` in the chart options but the chart container is empty. No JavaScript error — ECharts simply ignores unknown series types.

**Cause:** The 3D chart types require the **ECharts GL** extension (`echarts-gl.min.js`), which is a separate library not included in the base `echarts.min.js`. The registry only loads base ECharts when `viz: true` is set.

**Fix:** Add `gl: true` to the post's front matter alongside `viz: true`. This causes the build to add `tag-hash-gl` to the body class, which triggers the registry to load `echarts-gl.min.js` from CDN after base ECharts:

```yaml
viz: true
gl: true   # ← add this for any post using scatter3D, bar3D, surface, grid3D, etc.
```

**Template location:** `_layouts/default.html` (body class), `assets/js/viz-registry.js` (registry entry with CDN URL `https://cdn.jsdelivr.net/npm/echarts-gl@2/dist/echarts-gl.min.js`).

---

### Problem: Currency dollar signs render as broken math (KaTeX false-positive)

**Symptom:** A post containing dollar amounts like `$50 billion` or `$1.2 trillion` shows garbled text with math fonts, missing characters, or KaTeX error markup.

**Cause:** The KaTeX registry entry previously auto-detected any `$` character in page content and loaded the KaTeX renderer. Currency values triggered this incorrectly, causing KaTeX to try to parse prose text as LaTeX math expressions.

**Fix (build process):** The `detect` function in `assets/js/viz-registry.js` was changed to only auto-detect on `$$` (display math delimiters), not bare `$`. Posts with only inline math (`$x^2$`) must now opt in explicitly:

```yaml
math: true   # required for posts with LaTeX math — never rely on auto-detection
```

**Rule:** Any post that contains LaTeX — inline `$...$` or display `$$...$$` — should have `math: true` in front matter. The `$$` auto-detect remains as a fallback for display math blocks, but `math: true` is the reliable, unambiguous method.

---

### Problem: ECharts inline script doesn't render (viz loads but chart stays blank)

**Symptom:** Post has `viz: true` and a custom `<script type="module">` block with inline ECharts code, but the chart container remains empty.

**Cause (common):** The inline script accesses `echarts` before `core.js` has fetched it from CDN. `core.js` loads ECharts asynchronously; the inline script must wait.

**Cause (less common):** The script uses `typeof echarts !== 'undefined'` as the poll condition. This works, but `window.echarts` is the canonical form and avoids scope ambiguity inside IIFE wrappers.

**Fix:** Use the documented boilerplate at the top of every inline viz script:
...
Requirements:
- Script must be `type="module"` (enables top-level `await`)
- Post must have `viz: true` in front matter (tells core.js to load ECharts in the first place)
- Always use `window.echarts`, not bare `echarts`, to avoid ReferenceError in strict module scope

---

### Problem: Ground track chart shows satellite lines but no map background

**Symptom:** A post plots satellite ground tracks using ECharts' `geo` coordinate system, but the map background is blank grey with no country borders or land masses.

**Cause:** The post calls `echarts.registerMap('world', { type: 'FeatureCollection', features: [] })` with an empty GeoJSON — a placeholder that was never replaced with real world geometry.

**Fix (option A — use cartesian2d):** Convert the chart from `coordinateSystem: 'geo'` to a plain cartesian chart with longitude on the X axis (−180 to 180) and latitude on the Y axis (−90 to 90). The `[lon, lat]` data pairs map directly. Add `markLine` dashed reference lines for the equator and latitude limits. Remove the `registerMap` call entirely. This is the simpler fix and works well for educational content.

**Fix (option B — load real GeoJSON):** Fetch a real world GeoJSON from CDN inside the script and register it before building the chart option. Example source: `https://cdn.jsdelivr.net/npm/world-atlas@2/...` (requires topojson conversion) or a pre-converted GeoJSON. Adds network overhead (~200 KB) but preserves the geographic map context.
